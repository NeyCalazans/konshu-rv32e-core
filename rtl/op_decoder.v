...
module op_decoder (
    ...
    // Parameters for the ALU to decide wich operation to perform
    ...
    // Parameters for the immediate extend unit to decide how to extend it
    ...
    // Parameters that indicate what type of operation the op code represents
    localparam [4:0] R_TYPE        = 5'b01100;
    localparam [4:0] I_TYPE_LOAD   = 5'b00000;
    localparam [4:0] I_TYPE_ARITH  = 5'b00100;
    localparam [4:0] I_TYPE_JALR   = 5'b11001;
    localparam [4:0] J_TYPE_JAL    = 5'b11011;
    localparam [4:0] U_TYPE_LUI    = 5'b01101;
    localparam [4:0] U_TYPE_AUIPC  = 5'b00101;
    localparam [4:0] S_TYPE        = 5'b01000;  
    localparam [4:0] B_TYPE_BRANCH = 5'b11000;

    // ------------------------------------------
    // Logic
    // ------------------------------------------

    // Sign used to indicate if a jump instructions is going to be performed
    assign o_jump_ID      = (i_op == J_TYPE_JAL | i_op == I_TYPE_JALR) ? 1'b1 : 1'b0;  

    // Sign used to indicate if a branch instruction is going to be performed
    assign o_branch_ID    = (i_op == B_TYPE_BRANCH) ? 1'b1 : 1'b0;  

    // Sign that indicates if a memory write operation is going to be performed
    assign o_mem_write_ID = (i_op == S_TYPE) ? 1'b1 : 1'b0;  

    // Sign not beign used, check
    ...
    assign o_imm_src_ID = 
        (i_op == U_TYPE_LUI)    ? U_type : 
        (i_op == U_TYPE_AUIPC)  ? U_type : 
        (i_op == J_TYPE_JAL)    ? J_type :  
        (i_op == B_TYPE_BRANCH) ? B_type : 
        (i_op == S_TYPE)        ? S_type : 
        I_type;
