...
module mem_wb #(
    ...
    input wire                  clk;
    input wire                  rst;
    input wire [DATA_WIDTH-1:0] i_alu_result_M;  // Datapath
    input wire [DATA_WIDTH-1:0] i_read_data_M;
    input wire [DATA_WIDTH-1:0] i_pc_plus4_M;
    input wire [DATA_WIDTH-1:0] i_pc_target_M;
    input wire [REG_WIDTH-1:0]  i_rd_M;
    input wire                  i_reg_write_M;  // Control
    input wire [1:0]            i_result_src_M;
    input wire                  i_mem_write_M;
    input wire [DATA_WIDTH-1:0] i_write_data_M;
    input wire [2:0]            i_funct_3_M;

    output reg [DATA_WIDTH-1:0] o_alu_result_WB;  // Datapath
    output reg [DATA_WIDTH-1:0] o_alu_result_WB_neg;  // for write on Data Memory
    output reg [DATA_WIDTH-1:0] o_read_data_WB;
    output reg [DATA_WIDTH-1:0] o_pc_plus4_WB;
    output reg [DATA_WIDTH-1:0] o_pc_target_WB;
    output reg [REG_WIDTH-1:0]  o_rd_WB;
    output reg                  o_reg_write_WB;  // Control
    output reg [1:0]            o_result_src_WB;
    output reg                  o_mem_write_WB;
    output reg [DATA_WIDTH-1:0] o_write_data_WB;
    output reg [2:0]            o_funct_3_WB;
