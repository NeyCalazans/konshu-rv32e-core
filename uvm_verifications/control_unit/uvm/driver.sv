/**
 * @file driver.sv
 * @brief UVM driver that translates control-unit transactions into interface signal activity.
 *
...
/**
 * @class driver
 * @brief UVM driver for control-unit transactions.
 *
...
class driver extends uvm_driver #(pkt);
    ...
    virtual task drive_item (pkt pkt_item);
        @(posedge vif.clk);
        vif.op         <= pkt_item.i_op;
        vif.funct3     <= pkt_item.i_funct_3;
        vif.funct7_5   <= pkt_item.i_funct_7_5;
        vif.branch_ex  <= pkt_item.i_branch_EX;
        vif.jump_ex    <= pkt_item.i_jump_EX;
        vif.zero       <= pkt_item.i_zero;
        vif.enable     <= '1;
