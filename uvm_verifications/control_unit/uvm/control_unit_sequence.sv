/**
 * @file control_unit_sequence.sv
 * @brief UVM sequence that drives control-unit transactions until coverage reaches the target.
 *
 ...
 */
class control_unit_sequence extends uvm_sequence #(pkt);
    ...
    function new (string name = "control_unit_sequence");
        super.new(name);
    endfunction

    /// @brief Sends randomized control-unit transactions until the coverage collector reports 100%.
    virtual task body();
        pkt packet;
        `uvm_create(packet)
