/**
 * @file driver.sv
 * @brief UVM driver that translates extend-unit transactions into interface signal activity.
 *
 * @details Receives sequence items from the sequencer and drives the immediate
 *          format/raw immediate onto `s_interface` for one clock cycle each.
 * @ingroup uvm_components
 */
/**
 * @class driver
 * @brief UVM driver for extend-unit transactions.
 *
...
class driver extends uvm_driver #(pkt);
    ...
    /// @brief Drives one immediate-format/raw-immediate pair with `enable` pulsed for one clock cycle.
    virtual task drive_item (pkt pkt_item);
        @(posedge vif.clk);
        vif.imm_src <= pkt_item.i_imm_src_ID;
        vif.imm_id  <= pkt_item.i_imm_ID;
        vif.enable  <= '1;
