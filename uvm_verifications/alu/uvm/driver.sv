/**
 * @file driver.sv
 * @brief UVM driver that translates ALU transactions into interface signal activity.
 *
 * @details Receives sequence items from the sequencer and drives the ALU opcode
 *          and operands onto `s_interface` for one clock cycle each.
 * @ingroup uvm_components
 */
/**
 * @class driver
 * @brief UVM driver for ALU transactions.
 *
...
class driver extends uvm_driver #(pkt);
    ...
    /// @brief Drives one ALU opcode/operand pair with `enable` pulsed for one clock cycle.
    virtual task drive_item (pkt pkt_item);
        @(posedge vif.clk);
        vif.register_1 <= pkt_item.i_rd1_EX;
        vif.register_2 <= pkt_item.i_rd2_EX;
        vif.alu_ctrl <= pkt_item.i_alu_ctrl_EX;
        vif.enable <= '1;
