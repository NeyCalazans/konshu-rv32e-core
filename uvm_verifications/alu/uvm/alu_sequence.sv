/**
 * @file alu_sequence.sv
 * @brief UVM sequence that drives ALU transactions until coverage reaches the target.
 *
 ...
 */
class alu_sequence extends uvm_sequence #(pkt);
    ...
    /// @brief Sends randomized ALU transactions until the coverage collector reports 100%.
    virtual task body();
        pkt packet;
        `uvm_create(packet)
