...
module alu #(
    ...
    localparam logic [4:0] AND = 5'b00000; 
    localparam logic [4:0] OR = 5'b00001;   
    localparam logic [4:0] XOR = 5'b00010; 
    localparam logic [4:0] ADD = 5'b00011;  
    localparam logic [4:0] SUB = 5'b00100;  
    localparam logic [4:0] SLL = 5'b00101;  
    localparam logic [4:0] SRL = 5'b00110;  
    localparam logic [4:0] SLT = 5'b00111; 
    localparam logic [4:0] SLTU = 5'b01000;  
    localparam logic [4:0] SRA = 5'b01001;  
    localparam logic [4:0] BEQ = 5'b01010;      // 10 check -> the ALU computes A − B and looks at the flags. If Z is asserted, the result is 0, so A = B. Otherwise, A is not equal to B.
    localparam logic [4:0] BNE = 5'b01011;  
    localparam logic [4:0] BLT = 5'b01100;  
    localparam logic [4:0] BLTU = 5'b01101;  
    localparam logic [4:0] BGE = 5'b01110;  
    localparam logic [4:0] BGEU = 5'b01111;  
    localparam logic [4:0] LUI = 5'b10000;  
    localparam logic [4:0] AUIPC = 5'b10001;  
    localparam logic [4:0] FENCE = 5'b10010;    // 18 dummy (check if it is necessary)
    localparam logic [4:0] ECALL = 5'b10011;    // 19 dummy (check if it is necessary)
    localparam logic [4:0] EBREAK = 5'b10100;   // 20 dummy (check if it is necessary)
    ...
    always @(*) begin : invert_register_2
    ...
    always @(*) begin : operations
        case (i_alu_ctrl_EX)
            ...
            AND: begin
                ...
                o_equal_EX = 1'b0;  // todo: verify if this signal is not used on this 'case' @grouposiris
            end
            ...
            SLL: begin
                ...
                o_alu_result_EX = i_rd1_EX << i_rd2_EX[4:0]; 
                o_equal_EX = 1'b0;  // todo: verify if this signal is not used on this 'case' @grouposiris
            end
            ...
            SLT: begin 
                ...
                o_alu_result_EX = {{WIDTH - 1{1'b0}}, ($signed(i_rd1_EX) <  $signed(i_rd2_EX))};
            ...
            BLT: begin
                ...
                o_alu_result_EX = 'b0;
                // todo: which signal is used to take a branch, o_equal_EX? @grouposiris
                o_equal_EX = ($signed(i_rd1_EX) <  $signed(i_rd2_EX));
            end
            ...
            //     // *bits 11-0 of rd1 are set to 0.
            //     o_alu_result_EX = 'b0;  // original osiris described version (o_pc_target_EX is executed outside 'alu' block, it is on 'pc_target' block)
