/**
 * @file hazard_unit_sequence.sv
 * @brief UVM sequence that drives hazard-unit transactions until coverage reaches the target.
 *
 ...
 */
class hazard_unit_sequence extends uvm_sequence #(pkt);
    `uvm_object_utils(hazard_unit_sequence)
    ...
    function new (string name = "hazard_unit_sequence");
        super.new(name);
    endfunction

    /// @brief Sends randomized hazard-unit transactions until the coverage collector reports 100%.
    virtual task body();
        pkt packet;
        `uvm_create(packet)
