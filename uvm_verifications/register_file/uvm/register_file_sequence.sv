/**
 * @file register_file_sequence.sv
 * @brief UVM sequence that drives register-file transactions until coverage reaches the target.
 *
 ...
 */
class register_file_sequence extends uvm_sequence #(pkt);
    ...
    /// @brief Sends randomized register-file transactions until the coverage collector reports 100%.
    virtual task body();
        pkt packet;
        `uvm_create(packet)
